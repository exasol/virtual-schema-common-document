 ...
 * structure must fulfill the requirements of a valid DNF. A valid DNF consists of disjunctions that contain
 * conjunctions that then contain variables or negated variables.
 *
    ...
    private static class DisjunctionExtractor implements QueryPredicateVisitor {
        ...
        public void visit(final LogicalOperator logicalOperator) {
            if (logicalOperator.getOperator().equals(LogicalOperator.Operator.OR)) {
                final Set<DnfAnd> operands = logicalOperator.getOperands().stream().map(this::visitSecondLevel)
    ...
    private static class ConjunctionExtractor implements QueryPredicateVisitor {
        ...
        public void visit(final LogicalOperator logicalOperator) {
            if (logicalOperator.getOperator().equals(LogicalOperator.Operator.AND)) {
                final Set<DnfComparison> operands = logicalOperator.getOperands().stream().map(this::visitThirdLevel)
                        .collect(Collectors.toSet());
