...
public final class LogicalOperator implements QueryPredicate {
    ...
    public QueryPredicate simplify() {
        final Set<QueryPredicate> simplifiedOperands = this.operands.stream().map(QueryPredicate::simplify)
                .filter(operand -> !(operand instanceof NoPredicate)).collect(Collectors.toSet());
