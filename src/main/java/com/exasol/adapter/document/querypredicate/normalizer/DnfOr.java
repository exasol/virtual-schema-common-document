...
public class DnfOr {
    ...
    public QueryPredicate asQueryPredicate() {
        final Set<QueryPredicate> convertedOperands = this.operands.stream().map(DnfAnd::asQueryPredicate)
                .filter(dnfAnd -> !(dnfAnd instanceof NoPredicate)).collect(Collectors.toSet());
