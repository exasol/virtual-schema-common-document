...
public final class DnfAnd {
    ...
    public QueryPredicate asQueryPredicate() {
        final Set<QueryPredicate> convertedOperands = this.operands.stream().map(DnfComparison::asQueryPredicate)
                .filter(comparison -> !(comparison instanceof NoPredicate)).collect(Collectors.toSet());
