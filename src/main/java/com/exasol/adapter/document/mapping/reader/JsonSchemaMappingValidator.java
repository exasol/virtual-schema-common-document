...
class JsonSchemaMappingValidator {
    private static final String MAPPING_LANGUAGE_SCHEMA = "schemas/edml/v1.0.0.json";
    ...
    private void makeWrongSchemaExceptionMoreReadable(final ValidationException exception, final String fileName) {
        if (exception.getMessage().startsWith("#/$schema:")
            ...
            throwExceptionWithMappingName(fileName, exception.getPointerToViolation()
                    + " $schema must be set  to https://raw.githubusercontent.com/exasol/virtual-schema-common-document/master/src/main/resources/schemas/edml/v1.0.0.json");
        }
    }
