...
If you did not mark any column as key and the adapter could not detects an key column, it will add an `INDEX` column.
These columns contains the position of the element in the nested list.
